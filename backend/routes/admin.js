...
// Now delegates to sp_create_project stored procedure
...
try {
  ...
  const maxProjects = limits.projects ?? -1;

  // Call stored procedure
  await db.query('SET @pid=0, @err=""');
  await db.query(
    `CALL sp_create_project(?, ?, ?, ?, ?, ?, ?, ?, ?, @pid, @err)`,
    [
      req.user.tenant_id,
      name,
      description || null,
  ...
  const [[out]] = await db.query('SELECT @pid AS project_id, @err AS error');

  if (out.error) {
    const statusCode = out.error.includes('limit') ? 403 : 409;
    return res.status(statusCode).json({ success: false, error: out.error });
  }

  res.status(201).json({ success: true, message: 'Project created.', data: { project_id: out.project_id } });
} catch (err) {
  console.error(err);
...
// Now delegates to sp_create_task stored procedure
router.post('/tasks', async (req, res) => {
const { project_id, assigned_to, name, description, priority, deadline } = req.body;
if (!project_id || !name) {
  return res.status(400).json({ success: false, error: 'project_id and name are required.' });
}

try {
  await db.query('SET @tid=0, @err=""');
  await db.query(
    `CALL sp_create_task(?, ?, ?, ?, ?, ?, ?, @tid, @err)`,
    [
      project_id,
      req.user.user_id,
      assigned_to || null,
  ...
  const [[out]] = await db.query('SELECT @tid AS task_id, @err AS error');

  if (out.error) {
    return res.status(400).json({ success: false, error: out.error });
  }

  res.status(201).json({ success: true, message: 'Task created.', data: { task_id: out.task_id } });
} catch (err) {
  console.error(err);
