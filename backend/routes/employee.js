...
// NOTE: DB triggers now auto-update analytics_summary + activity_logs on task status changes.
...
// DB trigger trg_after_task_update auto-updates analytics_summary + activity_logs.
// We add a notification to the manager when a task is completed.
...
// Notify the manager when task is completed
if (status === 'completed') {
const [[task]] = await db.query(
`SELECT t.name AS task_name, p.tenant_id,
tm.manager_id,
CONCAT(u.first_name,' ',u.last_name) AS employee_name
FROM tasks t
JOIN projects p ON t.project_id = p.project_id
LEFT JOIN team_members tmem ON tmem.employee_id = t.assigned_to
LEFT JOIN teams tm ON tmem.team_id = tm.team_id
JOIN users u ON t.assigned_to = u.user_id
WHERE t.task_id = ?`,
[req.params.id]
);
if (task && task.manager_id) {
await db.query(
`INSERT INTO notifications (user_id, type, title, message, data)
VALUES (?, 'task_completed', 'Task Completed', ?, ?)`,
[
task.manager_id,
`${task.employee_name} completed "${task.task_name}".`,
JSON.stringify({ task_id: parseInt(req.params.id) })
]
);
}
}
...
if (msg.is_deleted) { msg.message = null; }
return msg;
});
...
const timeDiff = (Date.now() - new Date(msg.created_at)) / 1000 / 60;
if (timeDiff > 15) {
...
// ── DELETE /api/employee/attendance/today ─────────────────────────────────
